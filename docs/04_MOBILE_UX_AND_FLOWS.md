...
always-visible Today divider, month/date grouping, pull-to-refresh, and a compact Jump to today
action when today is outside the visible or loaded page set. The initial window contains the current
calendar month, plus the prior month's final seven days when today is day 1 through 7 or the next
month's first seven days when today is within the current month's final seven days. That initial
window and lazy paging are bounded to two calendar months before the current month through two
calendar months after it. Scrolling near either edge loads the next non-overlapping calendar range
until that five-month boundary is reached, and retains the loaded pages during refresh and
navigation. A safe-area-aware floating create action remains visible while scrolling. Long-pressing
