...
policy fails closed for new high or critical advisories and for expired exceptions. Temporary
exceptions must identify exact GitHub advisory IDs, document why the vulnerable path is not exposed,
and include a near-term review deadline in `mobile/scripts/audit-production.mjs`.
