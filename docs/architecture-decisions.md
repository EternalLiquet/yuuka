...
Planned Savings records are owner-scoped aggregates separate from paycheck allocation. A paycheck
entry may link to at most one Active Planned Savings record, but the entry still reserves only its
own paycheck contribution. Posted status creates one active contribution transaction for the entry
...
Same-record Posted contribution replacements lock the Planned Savings record and validate the final
prospective balance instead of requiring the old contribution to be fully reversible in isolation.
Cross-record reassignments lock touched records in stable UUID order before reversing and applying
contributions.

Current Planned Savings balances are derived from unreversed opening-balance, contribution, and
withdrawal transactions. The internal `sinking_funds` table stores metadata, state, order, and
target fields, not current balance totals. Withdrawals and withdrawal reversals are explicit
transaction rows and version-guarded workflows so mobile and controllers do not duplicate the
authoritative balance rules.

Planned Savings records store an integer `position` scoped to the owner. New records append to the
active order, and reorder requests must include every active record ID exactly once. Archived
records remain readable with transaction history and can be restored.

The `SINKING_FUND` entry type, `/sinking-funds` API path, and related field and persistence names
remain compatibility identifiers rather than user-facing terminology.
